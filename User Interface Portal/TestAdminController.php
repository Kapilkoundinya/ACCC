...
class AdminViewController extends Controller
{
	...
	//	dd($paperLevel);
		...
		//Append taskscompleted element to LabLocations array
		...
		//PieChart data at each Location, encoded in JSON format
		$pieChartData =[];
		...
		//Other Statisctics like toptrending, most active consultant, etc
		$other_statistics = $this->statistics($services_all_locations,$LabLocations);
	dd($other_statistics);	
	...
	private function statistics($services_all_locations,$LabLocations){
		...
		while($topservice = current($services_all_locations)){
			if($topservice == max($services_all_locations))
				$toptrending = key($services_all_locations);
			next($services_all_locations);
		}
		//Top trending location
		$finalTaskCount = 0;
		$finalLocationName = 'SCE';
		...
		foreach($LabLocations as $locationName => $locationData){
			...
			if(count($locationData)>0){
				...
				if($finalTaskCount < $initialTaskCount){
					$finalTaskCount = $initalTaskCount;
					$finalLocationName = $initialLocationName;
				}
				else{
